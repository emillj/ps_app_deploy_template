...
Try {
	...
	##*===============================================
        ...
        [string]$scriptSccmDetectionFile = $TRUE
        [string]$scriptSccmDetectionFileName = "$appName $appVersion"        # "$appName $appVersion" or "anythinyouwant"
        [string]$scriptVariable01 = 'Variable01'
        [string]$scriptVariable02 = 'Variable02'
        [string]$scriptVariable03 = 'Variable03'
        [string]$scriptVariable04 = 'Variable04'
        [string]$scriptVariable05 = 'Variable05'
        ##*===============================================
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        Execute-MSI -Action Install -Path "$dirFiles\application.msi" -Transform "BMA Settings.mst"
        Execute-MSI -Action Install -Path "$dirFiles\application.msi" -Parameters 'PS=server_alias PD=database_name /QN'
        Execute-MSI -Action Install -Path "$dirFiles\application.msi" -Parameters 'PS=server_alias PD=database_name /QB!'  # '!' in '/QB!' to eliminate cansel button.
        ...
        Execute-Process -Path "$envSystem32Directory\regsvr32.exe" -Parameters "/s filename.dll" -WorkingDirectory "$envProgramFilesX86\dllfolder"

        # MSP - Install msp patch.
        Execute-MSI -Action Patch -Path "$dirFiles\Patch.msp" -Parameters "/passive /norestart"
        ...
        # HKCU REGISTRY - Add and/or remove registry value in current user for all users.
        [scriptblock]$HKCURegistrySettings = {
            ...
            Remove-RegistryKey -Key 'HKCU\Software\Microsoft\Office\Word\Addins\ospStavaRexAddIn.coospStavaRexAddIn' -Name 'LoadBehavior' -SID $UserProfile.SID
            }
        ...
        Copy-File -Path "$dirFiles\Application.ico" -Destination "$envProgramFilesX86\Vendor\Application\Application.ico"
        ...
        Execute-Process -Path "$dirFiles\setacl.exe" -Parameters "-on `"$envProgramFiles\EDP`" -ot file -actn ace -ace `"n:S-1-5-4;p:change;s:y`""    # Authenticated users, full rights to folder.
        Execute-Process -Path "$dirFiles\setacl.exe" -Parameters '-on "HKEY_LOCAL_MACHINE\Software\Classes" -ot reg -actn ace -ace "n:S-1-5-4;p:full;s:y -ignoreerr'    # Authenticated users, fill rights to registy branch.
        ...
        # Set Organisation specific values. 'BMA','MBV','SBG', 'SBK','SEV'
        If ($BMA) {
	    ...
	    ElseIf ($MBV) {
		    If ($DebugDialogs) {Show-DialogBox -Title 'Info:' -Text "Variable `$MBV = TRUE" }
	        }
        ...
        ElseIf ($SBK) {
		    If ($DebugDialogs) {Show-DialogBox -Title 'Info:' -Text "Variable `$SBK = TRUE" }
	        }
	    ...
	    Else {
            If ($DebugDialogs) {Show-DialogBox -Title 'Info:' -Text "No ORG variable = TRUE, running defaault code" }
            <# Enable this to force use of Organisation variable!
        ...
        # Create SCCM detection file if $scriptSccmDetectionFile = TRUE
        if ($scriptSccmDetectionFile) {
            # Create SCCM detection file in ...\program files\SCCM_Detection\
            $fileNameTime = $currentDateTime -replace(":","-")
            $fileText = "$installTitle`r`n`r`n$currentDateTime`r`n`r`nThis file is used by SCCM to track application installations.`r`nDo not delete!"
            New-Folder -Path "$envProgramFiles\SCCM_Detection" -ContinueOnError $TRUE    # Create target folder
            $fileText | Out-File "$envProgramFiles\SCCM_Detection\$scriptSccmDetectionFileName.txt"

            # Schedule rename to .BAK at next reboot.
            Execute-Process -Path "$dirFiles\movefile.exe" -Parameters "/accepteula `"$envProgramFiles\SCCM_Detection\$scriptSccmDetectionFileName.txt`" `"$envProgramFiles\SCCM_Detection\$scriptSccmDetectionFileName ($fileNameTime).BAK`""  
        }
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# Delete SCCM Trigger files matching $envProgramFiles\SCCM_Detection\$scriptSccmDetectionFileName*.*
                Remove-Item "$envProgramFiles\SCCM_Detection\*" -recurse -include *$scriptSccmDetectionFileName*
...
$envTemp                                  Checks for the existence of environment variables in the following order and uses the first path found:
                                              ...
                                              •The path specified by the TEMP (e.g. C:\Users\{Username}\AppData\Local\Temp) environment variable.
...
$envOSVersionBuild                        Build portion of the OS version number (e.g. {major}.{minor}.{build}.{revision})Revision portion of the OS version number (e.g. {major}.{minor}.{build}.{revision})
$envOSVersionRevision                     Operating System
...
$SessionZero                              Is the current process currently in session
...
$regKeyDeferHistory                       The path in the registry where the defer history for the package being installed is
